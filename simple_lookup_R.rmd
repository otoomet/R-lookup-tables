...
Data tables appear to be surprisingly slow.  I guess this is because
`[.data.table` has quite a bit of overhead.  If this is the case, one may expect to see a substantial speed improvement with vectorized lookups.
...
for n in ns
    ...
    lk = randperm(n)
    # keys to be looked up--same keys just in different order
```
