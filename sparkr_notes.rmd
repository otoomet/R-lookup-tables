...
If the string that represents time is not given in your current time zone, you can
...
(unless your OS is set to the Kabul tz).  In this example, the
...
somewhat hard to interpret.  If we were working in UTC,
instead of `r Sys.timezone()`, `dateUTC` would correspond to time in
the local time zone for `date` in Kabul.  But as we are working in `r
Sys.timezone()`, this means that whatever time in UTC corresponds to
our `date`, that time would correspond to our `dateUTC` if given not
in UTC but in Kabul.
`to_utc_timestamp` has it's own issues:
...
_timestamp_.  This can easily be seen by comparing dates:
```{r, compareUTC}
dateDF %>%
   mutate(date = to_timestamp(column("string"), "MM/dd yyyy HH:mm:ss")) %>%
   mutate(dateUTC = to_utc_timestamp(column("date"), "Asia/Kabul")) %>%
   filter(column("dateUTC") == column("date")) %>%
   collect()
```
These timestamps are not equal, in fact _dateUTC_ is less than _date_.
