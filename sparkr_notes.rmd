...
ss <- sparkR.session(master = paste0("local[1]"),
                     sparkHome = SPARK_HOME,
                     appName = "spark demo")
                           # note: SparkR with capital S
ss <- sparkR.session(master = "local[2]", appName = "spark demo")
...
"local[2]", but no dedicated number is provided (2.3.1).
...
Pivoting is a way to transform data frame from long to wide format,
...
The result will be grouped by distinct values of the groups (this is
what the grouped data is about) vertically, and by distinct pivoting
values of the pivot column (this is what `colname` does) horizontally.  One may
also supply a pre-defined list of distinct pivoting values, it is
...
the `agg` function ([see](#aggregating-with-agg)).  The entries for
...
dfp <- data.frame(quarter=rbinom(10, 3, 0.3),
                  id = (100 + rbinom(10,4,0.5)) %>%
                     as.character(),
                  result=runif(10)) %>%
   createDataFrame()
dfp %>%
...
* distinct integer values will be converted to floats and you see
  values like `100.0` instead of `100`.  Convert the pivoting columns
  to characters.
