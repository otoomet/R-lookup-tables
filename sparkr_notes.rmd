...
SparkR (and to dplyr package and many other R tasks).  A good source to
...
## Creating new columns: column expressions

There are two functions for computing on columns: `withColumn` and
`mutate`.  The arguments of the former are:
```{r eval=FALSE}
...
existing columns and constants.  It may include mathematical and
string operations and other similar functions.  Take the example dataframe
```{r withColumn}
dfa %>%
   collect()
dfa %>%
...
Example:
```{r substrExample}
df <- data.frame(names=c("Barnier", "Beto")) %>%
   createDataFrame()
df %>% withColumn("x", substr(column("names"), 3, 20)) %>%
   collect()
```
