...
Local installation on linux is very easy and straightforward:

1.
[download the latest version](https://spark.apache.org/downloads.html)
2. decompress it into a suitable location (e.g. `$HOME/local`)
3. If needed, you can also set 
```bash
SPARK_HOME=$HOME/local/spark-x.x.x-bin-hadoop
```
or whatever is your exact installation directory.  However, the
location of spark home can also be set when initializing spark session
from within R.
