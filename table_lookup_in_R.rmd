...
```{r noMatchMatch, cache=TRUE, dependson=c("setup", "noMatch")}
...
```{r, include=FALSE, dependson=c("setup", "noMatchMatch")}
...
```{r timeMatrixMatch, cache=TRUE, dependson=c("setup", "matrixMatch")}
time <- foreach(n1 = n, .combine=c) %do% {
   dat <- DGP(n1)
...
```
And the results are:
```{r timeMatrixFmatch, cache=TRUE, dependson=c("matrixFmatch", "setup")}
time <- foreach(n1 = n, .combine=c) %do% {
   dat <- DGP(n1)
...
```
And the results are:
```{r timeMatrixMixedMatch, cache=TRUE, dependson=c("matrixMixedMatch", "setup")}
time <- foreach(n1 = n, .combine=c) %do% {
   dat <- DGP(n1)
...
```
And the results are:
```{r timeNestedEnv, cache=TRUE, dependson=c("nestedEnv", "setup")}
time <- foreach(n1 = n, .combine=c) %do% {
   dat <- envDGP(n1)
...
# For comparison: same process in python/pandas
