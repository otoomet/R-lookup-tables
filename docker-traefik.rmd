...
I new almost zero experience with web development, however, for a team
...
This had probably been an easy and quick task for experienced network
people, but for a newbie it took a few days of trials and errors and frutstration.  Hope
others will find some of my experience useful.


# How did we get there

In order to extract information from legal documents, we need
...
faster and less error-prone manual work.  The app gets its data from a
server.  This is why we need the API.
...
(Unfortunately I still have only a vague idea what this does...)
...
* **EXPOSE** tells the container that the port 7887 should be
  accessible from outside of it.
* and finally, there is the command to start the API.
...
Unfortunately, for me, it's documentation is much sparser, and loaded
with network terminology (well, what else can you expect from a
network proxy...).  There are number of easy intros available but I
have a few notable differences: first, I cannot use the port 8080 on server
for traefik webUI, at least not as easily, as this port is the only
...
For `docker stack deploy` we need `docker-compose.yml` configuration file.
...
* finally, `networks`.  Here I use a custom separately created network
  ...
  create it with the command `docker network creat -d overlay
  trafik-pubic`.  The _overlay_-driver is necessary.  Note also that
  if you create the external network, you have to list it in the
  separate network section (at the end here) as external.
